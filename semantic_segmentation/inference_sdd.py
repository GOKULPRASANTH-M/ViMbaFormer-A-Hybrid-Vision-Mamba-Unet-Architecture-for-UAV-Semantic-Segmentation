...
def sdd2rgb(mask):
    ...
    mask_rgb[np.all(mask_convert == 1, axis=0)] = [130, 76, 0]
    mask_rgb[np.all(mask_convert == 2, axis=0)] = [0, 102, 0]
    mask_rgb[np.all(mask_convert == 3, axis=0)] = [112, 103, 87]
    mask_rgb[np.all(mask_convert == 4, axis=0)] = [28, 42, 168]
    mask_rgb[np.all(mask_convert == 5, axis=0)] = [48, 41, 30]
    mask_rgb[np.all(mask_convert == 6, axis=0)] = [0, 50, 89]
    mask_rgb[np.all(mask_convert == 7, axis=0)] = [107, 142, 35]
    mask_rgb[np.all(mask_convert == 8, axis=0)] = [70, 70, 70]
    mask_rgb[np.all(mask_convert == 9, axis=0)] = [102, 102, 156]
    mask_rgb[np.all(mask_convert == 10, axis=0)] = [254,228,12]
    mask_rgb[np.all(mask_convert == 11, axis=0)] = [254,148,12]
    mask_rgb[np.all(mask_convert == 12, axis=0)] = [190,153,153]
    mask_rgb[np.all(mask_convert == 13, axis=0)] = [153,153,153]
    mask_rgb[np.all(mask_convert == 14, axis=0)] = [255,22,96]
    mask_rgb[np.all(mask_convert == 15, axis=0)] = [102,51,0]
    mask_rgb[np.all(mask_convert == 16, axis=0)] = [9,143,150]
    mask_rgb[np.all(mask_convert == 17, axis=0)] = [119,11,32]
    mask_rgb[np.all(mask_convert == 18, axis=0)] = [51,51,0]
    mask_rgb[np.all(mask_convert == 19, axis=0)] = [190,250,190]
    mask_rgb[np.all(mask_convert == 20, axis=0)] = [112,150,146]
    mask_rgb[np.all(mask_convert == 21, axis=0)] = [2,135,115]
...
def get_args():
    ...
    arg("-i", "--image_path", type=str, default='/home/srikanth/Interns/Gokul/Final_Model/patches/sdd_dataset/SDD_test/Seq1', help="Path to  huge image")
    ...
    arg("-d", "--dataset", help="dataset", default="sdd")
...
def main():
    ...
    for seq in seqs:
        ...
        for img_path in img_paths:
            ...
            if args.dataset == 'sdd':
                output_mask = sdd2rgb(output_mask)
            else:
