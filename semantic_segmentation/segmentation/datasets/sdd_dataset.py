...
ORIGIN_IMG_SIZE = (1024, 1024)  # After resizing from original 6000x4000
...
def train_aug(img, mask):
    crop_aug = albu.Compose([
        albu.RandomScale(scale_limit=(-0.5, 1.0)),  
        albu.RandomCrop(height=512, width=512)
    ])
    
    img, mask = np.array(img), np.array(mask)
    augmented = crop_aug(image=img, mask=mask)
...
class SDDDataset(Dataset):
    def __init__(self, data_root='data/sdd', mode='val', img_dir='images_1024', mask_dir='masks_1024',
