...
def label_smoothed_nll_loss1(
    ...
    """
    Modified version supporting class weights while maintaining original functionality
    """
    if target.dim() == lprobs.dim() - 1:
        target = target.unsqueeze(dim)
    ...
    if weight is not None:
        # Get original class indices (before smoothing)
        class_indices = target.squeeze(dim) if target.size(dim) == 1 else target.argmax(dim=dim)
        weight_matrix = weight[class_indices]
        
        nll_loss = nll_loss * weight_matrix
        smooth_loss = smooth_loss * weight_matrix.unsqueeze(dim)
    ...
    if reduction == "sum":
        ...
        if ignore_index is not None:
            nll_loss = nll_loss.sum() / (~pad_mask).float().sum()
            smooth_loss = smooth_loss.sum() / (~pad_mask).float().sum()
        else:
            nll_loss = nll_loss.mean()
            smooth_loss = smooth_loss.mean()
