...
from .soft_ce import SoftCrossEntropyLoss, SoftCrossEntropyLoss1
...
class UnetFormerLoss1(nn.Module):
    def __init__(self, ignore_index=255, class_weights=None):
        super().__init__()
        # Store class weights
        self.class_weights = class_weights
        
        # Initialize main loss components with weights
        self.main_loss = JointLoss(
            SoftCrossEntropyLoss1(smooth_factor=0.05, ignore_index=ignore_index, weight=class_weights),
            DiceLoss(smooth=0.05, ignore_index=ignore_index), 
            1.0, 1.0  # These are the weights for combining CE and Dice
        )
        
        # Initialize aux loss with weights
        self.aux_loss = SoftCrossEntropyLoss1(smooth_factor=0.05, ignore_index=ignore_index, weight=class_weights)
